...
view-app     → 主应用
    ...
    ├── page-module  功能模块页（9 个模块共用，动态渲染）
    ├── page-admin   后台管理页
    └── page-reader  博客阅读视图
```
...
| -------- | ---- | ---- | ------------ | ---------------- | -------- | -------- | -------- |
| personal | 个人 | 🧑    | 专注自我提升 | `#4F6EF7` 电感蓝 | 写下你的想法 | 9 个全部 | 待办 / 完成 / 习惯 / 目标 |
| couple   | 情侣 | 💑    | 共同成长     | `#F472B6` 樱花粉 | 写下你们的故事 | 9 个全部 | 待办 / 完成 / 习惯 / 番茄 |
| married  | 夫妻 | 👩❤️👨  | 共建家庭     | `#FB923C` 暖橙   | 经营你们的小家 | 8 个（隐藏番茄钟） | 待办 / 完成 / 结余 / 目标 |
| family   | 家庭 | 👨👩👧👦 | 全家共享     | `#34D399` 翠绿   | 记录全家人的生活 | 7 个（隐藏番茄钟、人物） | 待办 / 习惯 / 结余 / 文章 |
...
- **Cloudflare**：API 地址 + 同步密码（Worker 内 SHA-256 哈希校验）；`b_cloud` 持久化自动重连
...
| `b_version`    | 数据版本   | number（字符串存储），当前为 **2**          |
...
- **外部更新检测**：`file` 模式每 5s 轮询文件 mtime；`cloud` 模式每 15s 轮询 `updatedAt`；`s3` 模式每 15s 轮询 `Last-Modified` 响应头；判定"其他端更新"后全量重载并回写 localStorage。
- **冲突语义**：最后写入者胜（Syncthing 原生语义）；首次连接双方都有数据时弹窗让用户选择数据源，避免误覆盖。
- **云端 API**（`worker.js`）：`POST /api/setup`（首次设置同步密码，仅一次）/ `GET /api/data`（鉴权拉取）/ `PUT /api/data`（鉴权覆盖保存）；密码 SHA-256 哈希存储，全量数据存 KV 单键。
...
3. 若云端与本地都有数据 → 弹窗选择数据源；之后自动双向同步（15s 轮询）
...
**Q：账号密码存在云端吗？**
A：**不存**。登录账号密码（`b_auth`）只在本机浏览器（PBKDF2 哈希，每设备独立），云端只存两样：内容数据（KV 的 `data` 键）和同步密码的 SHA-256 哈希（`auth` 键）。设计原因：设备访问控制互不干扰、避免多设备互相覆盖登录凭据、减少云端暴露面。换设备时：新设备用默认凭据登录 → 设置本设备密码 → 连接云端拉取数据。如需要"云端统一账号"（服务端认证），见 §12.2 规划。

**Q：数据存在哪？安全吗？**
A：云端数据整体存于 KV 命名空间 `beryl-kv` 的 `data` 键（明文 JSON，含 `_meta` 元信息）。HTTPS 传输；读写需 Bearer 同步密码。介意明文可后续加 AES-GCM 加密（见 §12.2 规划）。

**Q：免费额度够用吗？**
A：见 §13.4 费用表——轮询 15s 约 5,760 次请求/天，远低于 10 万次/天的免费限额。
...
## 14. v2 架构规划（2026-08-14 定稿，阶段 1 已启动）

> 本文档维护 v1（单文件 `index.html`，稳定版继续可用）；v2 为新工程（`D:\dsharness\v2`），按 §10 更新日志 v2.x 记录进展。**v1 与 v2 并存，v2 阶段 1 为"功能平移"（行为一致），阶段 2 起能力升级。**
...
| 同步策略 | **按条 LWW**（每条记录 updatedAt 决胜，同毫秒 deviceId 字典序）+ 删除墓碑 + 增量传输 |
...
数据层：IndexedDB（Dexie）主存储 + 变更日志（append-only）+ 元数据
同步引擎：增量同步（游标）+ 按条 LWW + 墓碑；传输适配器（Cloudflare API/S3/文件导出）
...
POST /api/sync/pull { since }     → 返回游标之后的变更
```
...
| 2 | 数据层迁 IndexedDB（逐条+变更日志+旧数据迁移） | 待启动 |
| 3 | 同步引擎升级（增量+LWW+墓碑） | 待启动 |
| 4 | Worker 升级 D1 + 新同步 API | 待启动 |
| 5 | PWA + 加密 + 打磨 | 待启动 |
