...
const why = ref('')
const problem = ref('')
const desiredChange = ref('')
const progressEvidence = ref('')
const currentGap = ref('')
const nextTest = ref('')
...
async function create(): Promise<void> {
  ...
  const matter = await matterAsyncRepository.create({ title: title.value, why: why.value, problem: problem.value, desiredChange: desiredChange.value, progressEvidence: progressEvidence.value, currentGap: currentGap.value, nextTest: nextTest.value, stopCondition: stopCondition.value })
  title.value = ''; why.value = ''; problem.value = ''; desiredChange.value = ''; progressEvidence.value = ''; currentGap.value = ''; nextTest.value = ''; stopCondition.value = ''; showProblemFields.value = false; await refreshMatters()
...
<template>
  <div class="matters-page">
    <header class="page-head">
      <div><p class="eyebrow">我的课题</p><h1 class="font-title">我的课题</h1><p>围绕一个现实问题，决定今天最小必要行动。</p></div>
<details class="data-tools"><summary>数据工具</summary><div class="head-actions"><input ref="vaultInput" aria-label="选择 Open Vault 文件夹或文件" type="file" multiple accept=".md,.json" webkitdirectory directory hidden @change="importVault"><el-button plain size="small" @click="exportVault">导出 Open Vault</el-button><el-button plain size="small" @click="vaultInput?.click()">导入 Vault</el-button></div></details>
...
<div class="new-fields"><el-input v-model="title" aria-label="课题标题" size="large" placeholder="现在正在面对什么课题？" /><el-input v-model="why" aria-label="课题为什么值得处理" placeholder="为什么现在值得处理？（可选）" /><details v-model="showProblemFields"><summary>如果这是一个学习问题，补充解决闭环（可选）</summary><p class="field-hint">先写问题，再决定最小必要的学习；不为了“自律”制造学习。</p><div class="problem-fields"><el-input v-model="problem" type="textarea" aria-label="现实问题" placeholder="我正在解决什么现实问题？" /><el-input v-model="desiredChange" type="textarea" aria-label="期望变化" placeholder="我希望现实发生什么变化？" /><el-input v-model="progressEvidence" type="textarea" aria-label="进展证据" placeholder="什么证据说明正在变好？" /><el-input v-model="currentGap" type="textarea" aria-label="当前缺口" placeholder="现在卡在哪里？" /><el-input v-model="nextTest" type="textarea" aria-label="下一次验证" placeholder="下一次马上要试什么？" /><el-input v-model="stopCondition" type="textarea" aria-label="停止条件" placeholder="何时停止、换方法或停止学习？" /></div></details></div>
...
<div class="field-choice"><label><input :checked="manualDecisions[field.key] === 'keep-local'" aria-label="字段保留本地" type="radio" :name="'field-' + field.key" @change="setManualDecision(field.key, 'keep-local')">本地</label><label><input :checked="manualDecisions[field.key] === 'use-incoming'" aria-label="字段采用 Vault" type="radio" :name="'field-' + field.key" @change="setManualDecision(field.key, 'use-incoming')">Vault</label></div>
    ...
    <div class="toolbar"><div class="filters" role="group" aria-label="Matter 状态筛选"><button v-for="item in ([['active','进行中'],['paused','暂停'],['archived','已归档'],['all','全部']] as const)" :key="item[0]" :class="{ on: status === item[0] }" :aria-pressed="status === item[0]" @click="status = item[0]">{{ item[1] }} <small>{{ counts[item[0]] }}</small></button></div></div>
     <div v-if="items.length" class="matter-list"><button v-for="item in items" :key="item.calmyId" class="matter-card beryl-card hoverable" @click="router.push('/app/matters/' + item.calmyId)"><div class="card-top"><span class="stage">{{ item.currentStage }} · {{ item.trajectory }}</span><span class="state">{{ item.status }}</span></div><h2 class="font-title">{{ item.title }}</h2><p>{{ item.why || '还没有写下原因。' }}</p><div v-if="item.problem" class="problem-summary"><b>要解决的问题</b><span>{{ item.problem }}</span><small v-if="item.nextTest">下一次验证：{{ item.nextTest }}</small></div><div class="card-bottom"><span>revision {{ item.revision }}</span><b>打开 →</b></div></button></div>
...
.new-fields details{border-top:1px solid var(--c-border-soft);padding-top:8px}.new-fields summary{cursor:pointer;color:var(--c-text-2);font-size:11px}.field-hint{color:var(--c-text-3);font-size:10px;line-height:1.5}.problem-fields{display:grid;grid-template-columns:1fr 1fr;gap:7px;margin-top:8px}.problem-fields :deep(textarea){min-height:54px}.problem-summary{display:grid;gap:3px;margin:10px 0;padding:8px;border-left:2px solid var(--scene);background:var(--scene-soft);font-size:10px}.problem-summary b{color:var(--scene)}.problem-summary span{color:var(--c-text-2);line-height:1.5}.problem-summary small{color:var(--c-text-3)}@media(max-width:620px){.problem-fields{grid-template-columns:1fr}}
</style>
<style scoped>
