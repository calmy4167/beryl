...
<template>
  <div class="calendar-page">
    <header class="page-head"><div><p class="eyebrow">CALENDAR · TIME + GROWTH AXIS</p><h1 class="font-title">日历与成长</h1><p>日期承载现实证据，成长轴显示 Matter 与 Cycle 的方向。</p></div><div class="calendar-controls"><button @click="shiftMonth(-1)">←</button><button class="month-label" @click="goToday">{{ monthLabel }}</button><button @click="shiftMonth(1)">→</button></div></header>

    <div class="calendar-layout"><section class="month-panel beryl-card"><div class="week-row"><span v-for="weekday in weekdays" :key="weekday">{{ weekday }}</span></div><div class="month-grid"><button v-for="cell in monthCellsValue" :key="cell.date" class="calendar-cell" :class="{ outside: !cell.inMonth, selected: selectedDate === cell.date, today: cell.date === todayKey() }" @click="selectDate(cell.date)"><span>{{ cell.day }}</span><div v-if="hasEvidence(cell.date).actions.length || hasEvidence(cell.date).records.length || hasEvidence(cell.date).state" class="cell-dots"><i v-if="hasEvidence(cell.date).actions.length" class="action-dot"></i><i v-if="hasEvidence(cell.date).records.length" class="record-dot"></i><i v-if="hasEvidence(cell.date).state" class="state-dot"></i></div></button></div></section>
