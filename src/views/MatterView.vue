...
const problem = ref(''); const desiredChange = ref(''); const progressEvidence = ref(''); const currentGap = ref(''); const nextTest = ref(''); const stopCondition = ref('')
...
async function refresh(): Promise<void> {
  ...
  try {
    ...
    if (nextMatter) { why.value = nextMatter.why; contradiction.value = nextMatter.primaryContradiction; problem.value = nextMatter.problem || ''; desiredChange.value = nextMatter.desiredChange || ''; progressEvidence.value = nextMatter.progressEvidence || ''; currentGap.value = nextMatter.currentGap || ''; nextTest.value = nextMatter.nextTest || ''; stopCondition.value = nextMatter.stopCondition || '' }
    tick.value++
...
async function saveContext(): Promise<void> {
  ...
  try {
    ...
    const patch = { why: why.value, primaryContradiction: contradiction.value, problem: problem.value, desiredChange: desiredChange.value, progressEvidence: progressEvidence.value, currentGap: currentGap.value, nextTest: nextTest.value, stopCondition: stopCondition.value }
    if (target) await withSaveState(() => updateSharedMatterAsync(target.owner, target.ownerId, matter.value!.calmyId, patch, matter.value!.revision, currentCollaboratorId()))
    else await withSaveState(() => matterAsyncRepository.update(matter.value!.calmyId, patch, { expectedRevision: matter.value!.revision }))
...
<template>
  <div v-if="loading" class="empty beryl-card" role="status" aria-live="polite"><h2 class="font-title">正在读取 Matter…</h2></div>
  <div v-else-if="matter" class="matter-page">
    ...
    <div class="detail-grid"><main>
      <section class="panel beryl-card"><div class="panel-head"><div><p class="eyebrow">CONTEXT</p><h2 class="font-title">当前上下文</h2></div><button @click="saveContext">保存</button></div><p v-if="sharedWriteTargets.length" class="shared-write-note">共享写入：{{ sharedWriteTargets[0].title }} · 当前操作者有编辑权限，变更会进入共享审计。</p><label>为什么现在值得处理？<textarea v-model="why" :placeholder="matter.why || '写下一句话即可'" @focus="why = matter?.why || ''" /></label><label>当前主要矛盾<textarea v-model="contradiction" :placeholder="matter.primaryContradiction || '还可以在实践后补充'" @focus="contradiction = matter?.primaryContradiction || ''" /></label><details class="problem-driven-detail"><summary>问题驱动学习（可选）</summary><p class="field-hint">学习只服务于解决当前问题，并通过现实反馈决定继续、改法或停止。</p><label>现实问题<textarea v-model="problem" placeholder="我正在解决什么现实问题？" /></label><label>期望变化<textarea v-model="desiredChange" placeholder="我希望现实发生什么变化？" /></label><label>进展证据<textarea v-model="progressEvidence" placeholder="什么证据说明正在变好？" /></label><label>当前缺口<textarea v-model="currentGap" placeholder="现在卡在哪里？" /></label><label>下一次验证<textarea v-model="nextTest" placeholder="下一次马上要试什么？" /></label><label>停止条件<textarea v-model="stopCondition" placeholder="何时停止、换方法或停止学习？" /></label></details></section>
...
.problem-driven-detail{margin-top:16px;border-top:1px solid var(--c-border-soft);padding-top:12px}.problem-driven-detail summary{cursor:pointer;color:var(--scene);font-size:12px}.problem-driven-detail .field-hint{font-size:10px;color:var(--c-text-3);line-height:1.5}.problem-driven-detail label{margin-top:10px}
</style>
