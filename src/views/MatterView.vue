...
const stageTitle = ref(''); const stageElement = ref<Stage['element']>('wood')
...
const records = computed(() => {
  ...
  if (!matterId) return []
  const recordIds = new Set(listRealityDocuments({ types: ['record'] }).filter(item => item.matterId === matterId).map(item => item.id))
  return recordRepository.list().filter(item => recordIds.has(item.calmyId))
})
...
function selectCycle(cycle: Cycle): void { selectedCycleId.value = cycle.calmyId; actionStageId.value = '' }
...
function createStage(): void {
  ...
  try {
    ...
    stageTitle.value = ''; actionStageId.value = stage.calmyId; refresh(); ElMessage.success('Stage 已加入 Cycle')
...
function createAction(): void {
  ...
  try {
    const action = actionRepository.create({ title: actionTitle.value, date: actionDate.value, matterId: matter.value.calmyId, cycleId: selectedCycle.value.calmyId })
      ...
      <div v-if="selectedCycle" class="stage-area">
        ...
        <div v-if="stages.length" class="stage-list">
          <article v-for="stage in stages" :key="stage.calmyId" class="stage-row" :class="{ selected: actionStageId === stage.calmyId }">
            <button class="stage-select" @click="actionStageId = stage.calmyId"><span class="stage-element">{{ elementNames[stage.element] }}</span><span>{{ stage.title }}</span><small>{{ stageStatusNames[stage.status] }} · {{ stage.actionIds.length }} 个 Action</small></button>
