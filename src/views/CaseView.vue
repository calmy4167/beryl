...
type Finance = { id: string; category: string; note: string; amount: number; amountCents?: number; type: string; date?: string }
...
const quickType = ref<'person' | 'diary' | 'transaction' | 'post'>('person')
const quickTitle = ref(''); const quickNote = ref(''); const quickAmount = ref<number | null>(null)
...
function quickCreateResource() {
  if (!item.value || !quickTitle.value.trim()) { ElMessage.warning('先填写资源标题'); return }
  const title = quickTitle.value.trim(); const note = quickNote.value.trim(); let targetId = ''
  if (quickType.value === 'person') {
    targetId = createEntityId(); people.create({ id: targetId, name: title, title: note })
  } else if (quickType.value === 'post') {
    targetId = createEntityId(); posts.create({ id: targetId, title, content: note || title })
  } else if (quickType.value === 'diary') {
    targetId = todayKey(); const existing = diaries.find(targetId)
    if (existing) diaries.update(targetId, row => ({ ...row, content: [row.content, note || title].filter(Boolean).join('\n') }))
    else diaries.create({ date: targetId, content: note || title })
  } else {
    const amount = Number(quickAmount.value)
    if (!Number.isFinite(amount) || amount <= 0) { ElMessage.warning('财务记录需要填写大于 0 的金额'); return }
    targetId = createEntityId(); const cents = Math.round(amount * 100)
    finance.create({ id: targetId, type: 'expense', amount: cents / 100, amountCents: cents, category: title, note, date: fmtDate(Date.now()) })
  }
  caseRelationRepository.link(item.value.id, quickType.value === 'transaction' ? 'transaction' : quickType.value, targetId, 'earth')
  quickTitle.value = ''; quickNote.value = ''; quickAmount.value = null; refresh(); ElMessage.success('已创建并关联资源')
}
...
<template>
  ...
  <div v-else>
    <div class="top"><button class="back" aria-label="返回课题列表" @click="router.push('/app/cases')">←</button><div class="title-wrap"><small>现实课题</small><el-input v-model="item.title" class="title" @change="saveBasics" /></div><el-select :model-value="item.status" size="small" aria-label="课题状态" @update:model-value="setStatus"><el-option v-for="(_, key) in STATUS_LABEL" :key="key" :label="STATUS_LABEL[key as CaseStatus]" :value="key" /></el-select><el-button text type="danger" @click="deleteCase">删除</el-button></div>
    ...
    <section v-else-if="activePhase === 'earth'" class="beryl-card workspace"><h2>⛰️ 土 · 资源、事实与沉淀</h2><p class="hint">引用已有的人物、日记、财务记录和文章；不复制原始数据。</p><div class="add"><el-select v-model="relationType"><el-option label="人物" value="person" /><el-option label="日记" value="diary" /><el-option label="财务" value="transaction" /><el-option label="文章" value="post" /></el-select><el-select v-model="relationId" filterable placeholder="选择要关联的资料"><el-option v-for="target in availableTargets" :key="target.id" :label="target.label" :value="target.id" /></el-select><el-button @click="addRelation">关联</el-button></div><div class="quick-resource"><el-select v-model="quickType" aria-label="快速创建资源类型"><el-option label="人物" value="person" /><el-option label="日记" value="diary" /><el-option label="财务" value="transaction" /><el-option label="文章" value="post" /></el-select><el-input v-model="quickTitle" :placeholder="quickType === 'transaction' ? '分类' : '标题或名称'" @keyup.enter="quickCreateResource" /><el-input v-if="quickType === 'transaction'" v-model.number="quickAmount" type="number" min="0.01" step="0.01" placeholder="金额" /><el-input v-else v-model="quickNote" placeholder="补充内容（可选）" @keyup.enter="quickCreateResource" /><el-button type="primary" @click="quickCreateResource">快速创建并关联</el-button></div><p v-if="!linkedResources.length" class="hint">还没有关联资源。</p><div v-for="row in linkedResources" :key="row.link.id" class="line"><span>{{ row.label }}</span><el-button text size="small" @click="unlink(row.link.id)">移除关联</el-button></div><ContentEditor v-model="phaseNote" :rows="6" placeholder="补充事实、资料摘要、获得的成果…" /></section>
...
.quick-resource{display:flex;gap:8px;align-items:center;padding:10px;border:1px dashed var(--c-border);border-radius:8px;background:var(--c-bg-soft)}.quick-resource :deep(.el-select),.quick-resource :deep(.el-input){flex:1;min-width:120px}
@media(max-width:600px){.quick-resource{flex-wrap:wrap}.quick-resource :deep(.el-select),.quick-resource :deep(.el-input){min-width:140px}}
</style>
