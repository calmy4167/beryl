...
import { captureRepository, type AiSuggestion, type CaptureItem } from '@/domain/capture'
...
const captures = computed(() => {
  ...
  return captureRepository.list().filter(item => ids.has(item.calmyId))
})
const suggestions = computed(() => { void tick.value; return captureRepository.listSuggestions() })
const pendingSuggestions = computed(() => suggestions.value.filter(item => item.status === 'suggested'))

function refresh(): void { tick.value++ }
function latestSuggestion(capture: CaptureItem): AiSuggestion | undefined { return capture.suggestionIds.map(id => captureRepository.findSuggestion(id)).find(item => item !== undefined) }
...
function capture(): void {
  ...
  try {
    const item = captureRepository.create(body.value)
    captureRepository.suggest(item.calmyId)
    body.value = ''; refresh(); ElMessage.success('已保存原文，并生成一条本地 suggestion')
  } catch (error) { ElMessage.error(error instanceof Error ? error.message : 'Capture 保存失败') }
}
function accept(suggestion: AiSuggestion): void {
  ...
  try { const result = captureRepository.acceptSuggestion(suggestion.calmyId, 0, overrides); refresh(); ElMessage.success(`已接受 suggestion，写入 ${result.suggestion.acceptedEntityType}`) }
  catch (error) { ElMessage.error(error instanceof Error ? error.message : '接受 suggestion 失败') }
}
function reject(suggestion: AiSuggestion): void {
  try { captureRepository.rejectSuggestion(suggestion.calmyId); refresh(); ElMessage.info('已拒绝 suggestion，原文仍保留') }
...
<template>
  <div class="capture-page">
    <header class="page-head"><div><p class="eyebrow">CAPTURE · UNDERSTAND LATER</p><h1 class="font-title">先收下来</h1><p>原文先被保留，理解只是 suggestion；没有确认，不会写入事实。</p></div><div class="pending-count"><b>{{ pendingSuggestions.length }}</b><span>待确认</span></div></header>
    <section class="capture-box beryl-card"><div class="capture-mark">↓</div><div class="capture-input"><textarea v-model="body" placeholder="想到什么就写什么：一句话、一个链接、一个问题或一段事实。" @keydown.ctrl.enter.prevent="capture" @keydown.meta.enter.prevent="capture" /><div class="capture-footer"><small>本地规则理解 · 不上传 · Ctrl / ⌘ + Enter 保存</small><button @click="capture">保存并理解</button></div></div></section>
