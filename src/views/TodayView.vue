...
<template>
  <div class="today-page">
    ...
    <section class="record-panel beryl-card"><div class="panel-head"><div><p class="eyebrow">REALITY RECORD</p><h2 class="font-title">记录今天实际发生了什么</h2></div><span>先记录，不急着评价</span></div><div class="record-row"><el-input v-model="recordBody" type="textarea" :rows="2" placeholder="学习了多久、状态如何、哪里偏离了计划？" /><select v-model="recordType"><option value="fact">事实/观察</option><option value="negative">负向变化</option></select><select v-if="recordType === 'negative'" v-model="negativeImpact"><option value="waste">浪费</option><option value="escape">逃避</option><option value="retreat">退缩</option><option value="loss">损耗</option><option value="other">其他</option></select><select v-model="recordMatterId"><option value="">不关联 Matter</option><option v-for="item in matters" :key="item.calmyId" :value="item.calmyId">{{ item.title }}</option></select><button @click="addRecord">保存记录</button></div></section>
