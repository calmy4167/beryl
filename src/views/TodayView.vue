...
const actions = computed(() => { void tick.value; return actionRepository.listForDate(date) })
...
function setLoad(load: TodayLoad) { savePlan({ load }) }
...
function addRecord() { if (!recordBody.value.trim()) { ElMessage.warning('先写下今天实际发生了什么'); return }; recordRepository.create({ body: recordBody.value, matterId: recordMatterId.value || undefined }); recordBody.value = ''; recordMatterId.value = ''; ElMessage.success('已记录现实'); tick.value++ }
...
<template>
  <div class="today-page">
    ...
    <section class="opening beryl-card"><div class="panel-head"><div><p class="eyebrow">MORNING OPENING</p><h2 class="font-title">今天的三问</h2></div><button @click="saveOpening">保存方向</button></div><label>为什么今天值得做？<textarea v-model="why" placeholder="它向上连接哪个 Matter 或长期方向？" /></label><div class="two-col"><label>必须守住<textarea v-model="mustProtect" placeholder="吃饭&#10;睡眠&#10;工作责任" /></label><label>今天可以不做<textarea v-model="letGo" placeholder="新教程&#10;算法&#10;额外整理" /></label></div><div class="load-row"><span>今日承载</span><button v-for="item in ([['good','很好'],['normal','普通'],['tired','疲惫'],['bad','很差']] as const)" :key="item[0]" :class="{ on: plan.load === item[0] }" @click="setLoad(item[0])">{{ item[1] }}</button></div></section>

    <section class="action-create beryl-card"><div class="panel-head"><div><p class="eyebrow">MINIMUM ACTION</p><h2 class="font-title">添加今日行动</h2></div><span>核心行动最多 3 个</span></div><div class="create-row"><el-input v-model="actionTitle" placeholder="例如：完成 JwtService 最小实现" @keyup.enter="addAction" /><select v-model="selectedMatterId"><option value="">不关联 Matter</option><option v-for="item in matters" :key="item.calmyId" :value="item.calmyId">{{ item.title }}</option></select><button @click="addAction">添加</button></div></section>
