...
export function AppShell() {
  ...
  function openDrawer() {
    ...
    drawerReturnRef.current = moreTriggerRef.current ?? (document.activeElement instanceof HTMLElement ? document.activeElement : null)
    setDrawer(true)
  }
  function closeDrawer() {
    const returnTarget = drawerReturnRef.current ?? moreTriggerRef.current
    drawerOpenRef.current = false
    const restoreFocus = () => {
      const target = returnTarget && document.contains(returnTarget)
        ? returnTarget
        : moreTriggerRef.current ?? document.querySelector<HTMLElement>('.bottom-nav button[aria-label="更多导航"]')
      target?.focus()
    }
    restoreFocus()
    setDrawer(false)
