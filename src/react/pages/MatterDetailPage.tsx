  ...
  return (
    <div className="matter-detail">
      ...
      {matter.problem && <section className="beryl-card problem-driven-detail">
        ...
        <p className="field-hint">学习只在解决当前问题时发生，并通过现实反馈决定继续、改法或停止。</p>
        <dl>
          <dt>现实问题</dt><dd>{matter.problem}</dd>
          {matter.desiredChange && <><dt>期望变化</dt><dd>{matter.desiredChange}</dd></>}
          {matter.progressEvidence && <><dt>进展证据</dt><dd>{matter.progressEvidence}</dd></>}
          {matter.currentGap && <><dt>当前缺口</dt><dd>{matter.currentGap}</dd></>}
          {matter.nextTest && <><dt>下一次验证</dt><dd>{matter.nextTest}</dd></>}
          {matter.stopCondition && <><dt>停止条件</dt><dd>{matter.stopCondition}</dd></>}
        </dl>
      </section>}
    </div>
  )
