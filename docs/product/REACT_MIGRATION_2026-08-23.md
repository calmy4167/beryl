...
当前是“React 工作台 + Vue 回退代码”阶段。核心工作流以及主要模块已由 React 接管；动态、旧人物等低频模块继续按批次迁移。Vue、Vue Router、Element Plus 及旧入口暂不删除，避免影响仍依赖旧组件的边缘功能。

## 下一步

1. 完成动态、旧人物等剩余低频模块的 React 功能页；
2. 逐步删除 Vue 兼容桥、Vue Router 和 Element Plus，并继续按页面拆分首包；
3. 完成所有扩展页面的离线、导入导出、同步和可访问性回归。
