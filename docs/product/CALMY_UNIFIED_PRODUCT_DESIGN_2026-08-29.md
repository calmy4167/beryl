# Calmy Attention OS 统一产品与体验设计（迁移参考）

> 版本：2026-09-07 · 状态：迁移期执行参考
>
> 本文解释当前 Attention OS、四入口和 Matter 兼容实现。2026-09-19 起，当前产品本体以 [`CALMY_PRODUCT_DESIGN_2026-09-19.md`](CALMY_PRODUCT_DESIGN_2026-09-19.md) 与 D-018 为准。本文中与新总设计冲突的定位、主流程、信息架构和成功指标不再指导新增功能；仍有效的数据安全、AI 判断权、可访问性和现实出口规则继续作为迁移约束。实现事实与剩余任务分别以实现总档案和 `OPEN_WORK.md` 为准。
...
9. **问题高于学习**：不为学习或自律本身而学习；只在真实问题暴露信息或能力缺口时，取得当下足够的知识并立刻回到应用验证。
...
- 面对工作、关系、创作、探索或生活中的具体问题，希望按需学习并尽快用于现实，而不是维护一套学习计划。
...
- 以课程消费、知识收藏、学习时长或“成为更自律的人”为最终产出的学习产品；
...
Matter 不是 Todo 或抽象目标，而是某个真正进入生命、值得占据一部分注意力的现实事项。它可以是工作难题、关系张力、创作命题、好奇问题、探索假设、选择或生命阶段。“学习某主题”本身默认不构成 Matter；它必须连接到用户愿意处理的问题或希望验证的变化。

列表只显示：正在解决什么、为什么重要、怎样算有变化、这一轮和下一步。详情保留 Action、Record、Cycle、人物、资料和 Flow 专题上下文。暂停、归档和结束都是合法状态。

#### 问题驱动学习

学习统一作为 Matter 内的手段，采用以下闭环：
...
“问题”不等于狭窄功利任务；用户主动选择的好奇、创作和探索可以成为问题，只要能说明当前想弄清什么、下一步如何接触现实。纯粹休息、欣赏和生活不需要被包装成学习。

Matter 可按需呈现五个轻量字段：`problem`（正在处理什么）、`desired_change`（希望什么发生变化）、`evidence`（怎样知道有进展）、`current_gap`（此刻缺什么）、`next_test`（下一次如何使用或验证）。`stop_condition` 说明何时已经足够，避免学习无限展开。字段按需展开，不要求创建 Matter 时一次填完。

Today 中不推荐“学习 Java 2 小时”这类过程动作，优先改写为“读懂当前报错涉及的事务边界，并提交一个最小修复”或“查清两个方案的差异，今天作出选择”。学习时长只可作为容量预算或诊断证据，不代表问题取得进展。
...
Flow 继续作为二级“内容再生命系统”，现在统一归入 Attention Gate，而不是独立内容平台。原“学习模式”在用户界面统一改为“解题模式”；底层旧状态可为兼容保留，但不得继续塑造以学习为目的的流程。

- 内容首先来自用户当前问题关联的 Matter、Record、Seed、资料和历史应用证据；
- 外部来源必须可追溯、可关闭、可退出；
- 漫游、解题、回响和专题都必须有范围和结束卡；
- 解题开始前明确“希望得到的证据”和“应用位置”，专题开始前明确本批聚焦范围；范围只用于有限排序，不生成课程或长期学习计划；
- 收下、再见、用于当前问题、试一下、已足够、行动、关联和展开来源继续保留；旧“正在学习/已学会”只作为兼容状态映射；
- “行动”优先创建现实下一步并提示退出；
...
相同能力只保留一个主入口。设置与同步统一由 `/app/admin` 承载，完整 Vault / 实体迁移界面是该入口的组成部分；旧 `/app/admin/advanced` 只为已有书签保留，不显示另一套设置体验。

所有 React 页面与该完整管理页共享颜色、表面层级、表单焦点、卡片、主按钮、标签页、空状态、动效和窄屏间距规则。视觉调整不应通过复制页面、复制数据或新增独立同步状态完成。每个页面仍应清楚显示当前对象、一个主要行动、保存或同步状态，以及离开或返回上一级的路径。
...
飞书多维表格适配器是 2026-09-19 新增的实验外部数据源：第一阶段可将现有 `项目 / 任务 / 周报 / 成员` 映射为 Calmy 的事项、行动、复盘和人物上下文，但不改变 IndexedDB 的当前本地主事实源裁决。只有在真实权限、数据往返、字段演进、错误恢复和用户试用完成后，才重新评估是否允许飞书成为某个工作区的主数据源；浏览器不得直接持有飞书 App Secret。

2026-09-19 的 [飞书生活工作台规划](FEISHU_LIFE_WORKSPACE_PLAN_2026-09-19.md)对应 D-014（proposed）：建议飞书模式采用今天、项目、回顾与全局快速新建，按工作区统一来源，并将个人今日选择、界面偏好、派生视图和缓存与飞书主记录明确区分。该建议尚未替换本文件的四入口与本地主事实源；下一阶段按 OW-19 先完成字段映射、线上真实往返和核心页面来源统一，目标是降低重复维护与生活使用负担。
...
- 主动降载、暂停、放下或重写下一步的比例；
- 用户能否说明为什么这件事值得注意；
- 用户能否说清正在解决的问题、当前缺口、所学内容将用在哪里，以及什么证据表示已经足够；
- 从提出问题到第一次应用或验证的时间，以及应用后获得有效现实反馈的比例；
- 数据离线恢复、导出和重新导入成功率；
...
- 不以学习时长、课程完成量、答题数、知识收藏量或“更自律”的自我评价证明价值；
...
- 所有主行动都提供明确现实出口。
- 新建学习类 Matter 时引导用户改写为“正在解决的问题 + 第一次应用证据”；Today 不以纯时长学习动作作为默认推荐。
...
- 再开放 Focus 和漫游、解题、回响、专题四种模式；
- 最后根据试点证据加入可解释推荐；
- 不以 Flow 挤压 Today、Capture、Matters、Review。

退出门槛：每次 Flow 会话有明确问题或主动探索意图、边界、来源、应用动作和现实出口；用户可以选择“已足够”并停止继续推荐。
...
- 学习时长、连续天数、课程完成或自律评分成为默认目标，内容消费无法连接到具体问题和应用证据；
...
| IndexedDB / D1 / Markdown / Feishu 边界 | 保留现有事实源；Feishu 仅作为实验外部适配器，不因接入自动新增第二套事实源 |
| 学习模块、学习时长与自律目标 | 降级为 Matter 内按需行动和可选诊断信号；默认使用问题、缺口、应用证据与停止条件 |
| Flow 学习模式与“学会”动作 | UI 改为解题模式和“用于当前问题 / 试一下 / 已足够”；底层旧枚举仅为迁移兼容 |
...
- 它保护还是消耗注意力？
- 它是否更快把用户送回现实？
- 如果包含学习，它正在解决什么问题、填补什么缺口、将在哪里使用、怎样知道已经足够？
- 它是否尊重身体事实和低承载状态？
- 它是否帮助用户判断，而不是替用户判断？
- 它是否允许跳过、关闭、结束和放下？
...
- 它的长期维护成本是否低于现实收益？
- 它是否避免把时长、连续性、收藏或课程完成误写成现实进展？

任一核心问题无法回答，应先留在实验或拒绝进入开发。

## 22. 文档维护规则

- 本文是 Attention OS 现有实现和迁移约束的参考，不再是当前产品本体的统一入口。
- 当前产品定位、核心体验与长期迁移方向以 `CALMY_PRODUCT_DESIGN_2026-09-19.md` 为准。
- `PRODUCT_DECISIONS_2026-08-19.md` 继续追加具体 Accepted/Experiment 决策，不复制整篇设计。
- `OPEN_WORK.md` 只记录未完成实施与验收，不重复产品愿景。
- 旧产品重设计、UX、路线图、评审纪要和 2026-08-19 参考文档保留为来源与细节参考；涉及产品本体时服从 `CALMY_PRODUCT_DESIGN_2026-09-19.md` 与 D-018，涉及当前兼容实现时再使用本文细节。
